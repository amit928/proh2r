...
export default function SeperationReport(props) {
    ...
    function populateBarChart() {
        ...
        MAINDATA.graphDept.forEach((item, index) => {
            ...
            let tempData = {
                values: [parseInt(item?.requestCount)],
                label: item?.deptName,
                config: {
                    drawValues: false,
                    colors: [processColor(returnUniqueColorCode())],
                }
            }
    ...
    function populatePieChart() {
        ...
        MAINDATA.graphBand.forEach((item, index) => {
            ...
            let tempData = { value: parseInt(item.requestCount), label: item.bandName, name: item.bandName }
                ...
                <ScrollView refreshControl={
                    ...
                    <View style={{ flex: 1 }}>
                        ...
                        <View style={{ alignItems: 'center', justifyContent: 'center' }}>
                            ...
                            <Text  allowFontScaling={false} > Total Seperation Application By Department </Text>
                        ...
                        <View style={{ alignItems: 'center', justifyContent: 'center' }}>
                            ...
                            <Text  allowFontScaling={false} > Total Seperation Application By Bands </Text>
                        ...
                        <TitleComponent title='Seperation Report' />
                        ...
                        {MAINDATA.filterData.map((item, index) => {
                            ...
                            }} title={item.empName} statusMain={item.empCode} statusHeading={'Emp Code: '}
                                fromTo={Utility.rawDateToDDMMYYYY(item.raisedOn)}
                            />
                            )
                        })
                    ...
                    <View
                        ...
                        <View style={{ flex: 1, width: '100%', borderTopStartRadius: 20, borderTopEndRadius: 20, justifyContent: 'center', alignItems: 'center', overflow: 'hidden', }}>
                            ...
                            <View style={{ display: 'flex', width: '100%', height: 50, backgroundColor: '#ffff', alignItems: 'center', justifyContent: 'center', borderBottomWidth: 0.7, borderColor: '#d7d0e1' }}>
                                <Text  allowFontScaling={false}  style={{ alignItems: 'center', justifyContent: 'center', fontSize: 17, color: 'black', fontWeight: '500', }}>{'View Application'}</Text>
                            </View>
                            ...
                            <ScrollView style={{
                                ...
                                {Object.keys(selectedEmployeeDetail).length != 0 ?
                                    ...
                                    <>
                                        ...
                                        <CustomTextField editable={false} label='Separation ID' value={String(selectedEmployeeDetail?.empCode)} />

                                        <CustomTextField editable={false} label='Emp Code' value={String(selectedEmployeeDetail?.empCode)} />

                                        <CustomTextField editable={false} label='Name' value={String(selectedEmployeeDetail?.empName)} />

                                        <CustomTextField editable={false} label='Raise On Date' value={String(selectedEmployeeDetail?.raisedOn).split('T')[0]} />

                                        <CustomTextField editable={false} label='Separation Type' value={String(selectedEmployeeDetail?.seperationTypes?.seperationType)} />

                                        {/* Separation Type 	Releaving Date 	L1 Approval Date 	L2 Approval Date 	HR Manager Approval Date 	Resignation Status 	Exit Interview 	Asset Deallocation  */}
                                        <CustomTextField editable={false} label='Releaving Date' value={String(Utility.convertToDDMMYYYY(selectedEmployeeDetail?.releasingDate))} />

                                        <CustomTextField editable={false} label='L1 Approval Date' value={selectedEmployeeDetail?.L1ApprovalDate} />

                                        <CustomTextField editable={false} label='L2 Approval Date' value={selectedEmployeeDetail?.L2ApprovalDate} />

                                        <CustomTextField editable={false} label='HR Manager Approval Date' value={String( Utility.rawDateToDDMMYYYY( selectedEmployeeDetail?.HrManagerApprovalDate))} />

                                        <CustomTextField editable={false} label='Resignation Status' value={String(selectedEmployeeDetail?.resignationStatus)} />

                                        <CustomTextField editable={false} label='Asset Deallocation' value={String(selectedEmployeeDetail?.assetDeallocation)} />
